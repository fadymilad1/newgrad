...
export default function HospitalChatWidget({
  ...
  useEffect(() => {
    setTenantSubdomain(deriveTenantSubdomain(subdomain || siteSubdomain))
  ...
  const handleSend = async (event: React.FormEvent) => {
    ...
    try {
      ...
      syncAssistantMessage(assistantMessageId, (current) => ({
        ...current,
        content: 'I could not complete that request right now. Please try again in a moment or contact the hospital directly for urgent concerns.',
        assistant: {
          ...current.assistant,
          disclaimer: FALLBACK_DISCLAIMER,
